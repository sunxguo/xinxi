...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 学习成绩管理 <span class="c-gray en">&gt;</span> 学习成绩 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
	<div class="text-c"><!--  注册时间：
		...
		<select id="gender" class="select" name="admin-role" size="1" style="height:31px;width:inherit;vertical-align:middle;">
		 ...
		 挂科：
		<select id="unpass" class="select" name="admin-role" size="1" style="height:31px;width:inherit;vertical-align:middle;">
			<option value="-1" <?php echo !isset($_GET['unpass'])?'selected':'';?>>不考虑</option>
			<option value="1" <?php echo isset($_GET['unpass']) && $_GET['unpass']=='1'?'selected':'';?>>挂1科</option>
		...
		<button onclick="searchScore();" type="submit" class="btn btn-success radius" name=""><i class="Hui-iconfont">&#xe665;</i> 查询数据</button>
	</div>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				<td><input type="checkbox" value="<?php echo $sc->id;?>" name="id"></td>
				<td><?php echo $students[$sc->stu_id]->academyInfo->name;?></td>
				<td><?php echo isset($students[$sc->stu_id]->classInfo->name)?$students[$sc->stu_id]->classInfo->name:"";?></td>
				<td><?php echo $students[$sc->stu_id]->number;?></td>
				<td><?php echo $students[$sc->stu_id]->name;?></td>
				<td><?php echo $students[$sc->stu_id]->gender==0?'男':'女';?></td>
				<td><?php echo $students[$sc->stu_id]->specialityInfo->name;?></td>
				<td><?php echo $students[$sc->stu_id]->specialityInfo->length;?></td>
				<td><?php echo $students[$sc->stu_id]->address;?></td>
				<td><?php echo $students[$sc->stu_id]->nation;?></td>
				<td><?php echo $students[$sc->stu_id]->property;?></td>
				<td><?php echo $courses[$sc->course]->code;?></td>
				<td><?php echo $courses[$sc->course]->name;?></td>
				<td><?php echo $sc->bigclass;?></td>
				<td><?php echo $sc->score_ordinary;?></td>
				<td><?php echo $sc->score_term;?></td>
				<td><?php echo $sc->score;?></td>
				<td><?php echo $sc->examproperty;?></td>
				<td><?php echo $semesters[$sc->semester]->name;?></td>
				<?php /*
				...
				<td class="td-manage">
					...
					<?php endif;*/?>
					<a title="编辑" href="javascript:;" onclick="member_edit('修改成绩数据','/admin/scoreEdit','<?php echo $sc->id;?>','500','510')" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6df;</i>
					</a> 
					...
					<a title="删除" href="javascript:;" onclick="member_del(this,'<?php echo $sc->id;?>')" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6e2;</i>
					</a>
				</td>
			</tr>
...
function member_del(obj,id){
	layer.confirm('确认要删除该条数据吗？',function(index){
	    ...
	    score.id = id;
		dataHandler('/common/deleteInfo',score,null,null,null,function(){
...
function member_del_bulk(){
	...
	layer.confirm('确认要删除这些成绩数据吗？',function(index){
	    ...
	    scores.infoType = 'scores';
	    scores.idArray = memberArray;
	    dataHandler("/common/deleteBulkInfo",scores,null,null,null,function(){
	    	$("input[name='id']:checked").each(function(){
