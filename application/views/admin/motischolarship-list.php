...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 奖助管理 <span class="c-gray en">&gt;</span> 励志奖学金 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
	<div class="text-c"><!--  注册时间：
		...
		<button onclick="searchMotiScholarship();" type="submit" class="btn btn-success radius" name=""><i class="Hui-iconfont">&#xe665;</i> 查询数据</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="member_add('上传励志奖学金Excel','/admin/excelAdd?type=motischolarship&warnning=确定添加所有的励志奖学金数据项到数据库吗？','500','250')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 上传励志奖学金Excel</a>
		</span> 
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php foreach($motischolarship as $moti):?>
			<tr class="text-c">
				<td><input type="checkbox" value="<?php echo $moti->id;?>" name="id"></td>
				<td><?php echo isset($students[$moti->stu_id]->academyInfo->name)?$students[$moti->stu_id]->academyInfo->name:"";?></td>
				<td><?php echo isset($students[$moti->stu_id]->specialityInfo->name)?$students[$moti->stu_id]->specialityInfo->name:"";?></td>
				<td><?php echo $students[$moti->stu_id]->name;?></td>
				<td><?php echo $students[$moti->stu_id]->number;?></td>
				<td><?php echo $students[$moti->stu_id]->idnumber;?></td>
				<td><?php echo $students[$moti->stu_id]->gender=="0"?"男":"女";?></td>
				<td><?php echo $students[$moti->stu_id]->nation;?></td>
				<td><?php echo $students[$moti->stu_id]->enrolTime;?></td>
				<?php /*
				...
				<td class="td-manage">
					...
					<a title="删除" href="javascript:;" onclick="member_del(this,'<?php echo $moti->id;?>')" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6e2;</i>
					</a>
				</td>
			</tr>
...
function member_del(obj,id){
	layer.confirm('确认要删除该条数据吗？',function(index){
		var moti = new Object(); 
	    moti.infoType = 'motischolarship';
	    moti.id = id;
		dataHandler('/common/deleteInfo',moti,null,null,null,function(){
...
function member_del_bulk(){
	...
	layer.confirm('确认要删除这些数据吗？',function(index){
	    var motis = new Object();
	    motis.infoType = 'motis';
	    motis.idArray = memberArray;
	    dataHandler("/common/deleteBulkInfo",motis,null,null,null,function(){
	    	$("input[name='id']:checked").each(function(){
