...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 奖助管理 <span class="c-gray en">&gt;</span> 奖学金 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
	<div class="text-c"><!--  注册时间：
		...
		<select id="gender" class="select" name="admin-role" size="1" style="height:31px;width:inherit;vertical-align:middle;">
		 ...
		 获奖：
		 <select id="scholarshiptype" class="select" name="admin-role" size="1" style="height:31px;width:inherit;vertical-align:middle;">
			<option value="-1" <?php echo !isset($_GET['scholarshiptype'])?'selected':'';?>>所有</option>
			<option value="0" <?php echo isset($_GET['scholarshiptype']) && $_GET['scholarshiptype']=='0'?'selected':'';?>>思想品德奖学金</option>
			<option value="1" <?php echo isset($_GET['scholarshiptype']) && $_GET['scholarshiptype']=='1'?'selected':'';?>>学习奖学金</option>
			<option value="2" <?php echo isset($_GET['scholarshiptype']) && $_GET['scholarshiptype']=='2'?'selected':'';?>>社会实践奖学金</option>
			<option value="3" <?php echo isset($_GET['scholarshiptype']) && $_GET['scholarshiptype']=='3'?'selected':'';?>>体育奖学金</option>
			<option value="4" <?php echo isset($_GET['scholarshiptype']) && $_GET['scholarshiptype']=='4'?'selected':'';?>>济困奖学金</option>
			<option value="5" <?php echo isset($_GET['scholarshiptype']) && $_GET['scholarshiptype']=='5'?'selected':'';?>>纯济困奖学金</option>
			<option value="6" <?php echo isset($_GET['scholarshiptype']) && $_GET['scholarshiptype']=='6'?'selected':'';?>>优秀学生奖学金</option>
		</select>
		等级：
		 <select id="scholarshipgrade" class="select" name="admin-role" size="1" style="height:31px;width:inherit;vertical-align:middle;">
			<option value="-1" <?php echo !isset($_GET['scholarshipgrade'])?'selected':'';?>>所有</option>
			<option value="0" <?php echo isset($_GET['scholarshipgrade']) && $_GET['scholarshipgrade']=='0'?'selected':'';?>>一等奖</option>
			<option value="1" <?php echo isset($_GET['scholarshipgrade']) && $_GET['scholarshipgrade']=='1'?'selected':'';?>>二等奖</option>
			<option value="2" <?php echo isset($_GET['scholarshipgrade']) && $_GET['scholarshipgrade']=='2'?'selected':'';?>>三等奖</option>
		</select>
		 <select id="scholarshiptime" class="select" name="admin-role" size="1" style="height:31px;width:inherit;vertical-align:middle;">
			<option value="-1" <?php echo !isset($_GET['scholarshiptime'])?'selected':'';?>>所有</option>
			<!-- <option value="0" <?php echo isset($_GET['scholarshiptime']) && $_GET['scholarshiptime']=='0'?'selected':'';?>>0</option> -->
			<option value="1" <?php echo isset($_GET['scholarshiptime']) && $_GET['scholarshiptime']=='1'?'selected':'';?>>1</option>
			<option value="2" <?php echo isset($_GET['scholarshiptime']) && $_GET['scholarshiptime']=='2'?'selected':'';?>>2</option>
			<option value="3" <?php echo isset($_GET['scholarshiptime']) && $_GET['scholarshiptime']=='3'?'selected':'';?>>3</option>
			<option value="4" <?php echo isset($_GET['scholarshiptime']) && $_GET['scholarshiptime']=='4'?'selected':'';?>>4</option>
		</select> 次 &nbsp;&nbsp;&nbsp;
		...
		<button onclick="searchScholarship();" type="submit" class="btn btn-success radius" name=""><i class="Hui-iconfont">&#xe665;</i> 查询数据</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="member_add('上传奖学金Excel','/admin/excelAdd?type=scholarship&warnning=确定添加所有的奖学金数据项到数据库吗？','500','250')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 上传奖学金Excel</a>
		</span> 
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="90">学号</th>
				<!-- <th width="150">二维码</th> -->
				<th width="40">是否国助生</th>
				<th width="40">思想品德奖学金</th>
				<th width="40">学习奖学金</th>
				<th width="40">社会实践奖学金</th>
				<th width="40">体育奖学金</th>
				<th width="40">济困奖学金</th>
				<th width="40">纯济困奖学金</th>
				<th width="40">优秀学生奖学金</th>
				<th width="40">金额</th>
				<th width="130">体育奖学金(备注)</th>
				<th width="130">社会实践奖学金(备注)</th>
				<th width="40">是否双培生</th>
				<th width="100">操作</th>
			</tr>
		</thead>
		<tbody>
			<?php foreach($scholarship as $scho):?>
			<tr class="text-c">
				<td><input type="checkbox" value="<?php echo $scho->id;?>" name="id"></td>
				<td><?php echo isset($students[$scho->stu_id]->academyInfo->name)?$students[$scho->stu_id]->academyInfo->name:"";?></td>
				<td><?php echo $students[$scho->stu_id]->name;?></td>
				<td><?php echo $students[$scho->stu_id]->classInfo->name;?></td>
				<td><?php echo $students[$scho->stu_id]->number;?></td>
				<td><?php echo $scho->isnationass=="1"?"是":"否";?></td>
				<td><?php echo $scho->moral;?></td>
				<td><?php echo $scho->study;?></td>
				<td><?php echo $scho->practice;?></td>
				<td><?php echo $scho->sport;?></td>
				<td><?php echo $scho->jikun;?></td>
				<td><?php echo $scho->chunjikun;?></td>
				<td><?php echo $scho->youxiu;?></td>
				<td><?php echo $scho->total;?></td>
				<td><?php echo $scho->sportnote;?></td>
				<td><?php echo $scho->practicenote;?></td>
				<td><?php echo $students[$scho->stu_id]->isshuangpeisheng=="1"?"是":"否";?></td>
				<?php /*
				...
				<td class="td-manage">
					...
					<a title="删除" href="javascript:;" onclick="member_del(this,'<?php echo $scho->id;?>')" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6e2;</i>
					</a>
				</td>
			</tr>
...
function member_del(obj,id){
	layer.confirm('确认要删除该条数据吗？',function(index){
		var scho = new Object(); 
	    scho.infoType = 'scholarship';
	    scho.id = id;
		dataHandler('/common/deleteInfo',scho,null,null,null,function(){
...
function member_del_bulk(){
	...
	layer.confirm('确认要删除这些数据吗？',function(index){
	    var schos = new Object();
	    schos.infoType = 'schos';
	    schos.idArray = memberArray;
	    dataHandler("/common/deleteBulkInfo",schos,null,null,null,function(){
	    	$("input[name='id']:checked").each(function(){
