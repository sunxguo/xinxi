...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 消费情况 <span class="c-gray en">&gt;</span> 贫困生消费 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
	<div class="text-c"><!--  注册时间：
		...
		<select id="gender" class="select" name="admin-role" size="1" style="height:31px;width:inherit;vertical-align:middle;">
		 ...
		 消费金额范围：
		<input type="text" value="<?php echo isset($_GET['explower'])?$_GET['explower']:'';?>" id="explower" class="input-text" style="width:50px" placeholder="0" name=""> -
		<input type="text" value="<?php echo isset($_GET['expupper'])?$_GET['expupper']:'';?>" id="expupper" class="input-text" style="width:50px" placeholder="∞" name="">
		 消费排名：
		<select id="exprank" class="exprank" name="admin-role" size="1" style="height:31px;width:inherit;vertical-align:middle;">
			<option value="-1" <?php echo !isset($_GET['exprank'])?'selected':'';?>>所有</option>
			<option value="0" <?php echo isset($_GET['exprank']) && $_GET['exprank']=='0'?'selected':'';?>>前20%</option>
			<option value="1" <?php echo isset($_GET['exprank']) && $_GET['exprank']=='1'?'selected':'';?>>后20%</option>
		</select>
		...
		<button onclick="searchExpense();" type="submit" class="btn btn-success radius" name=""><i class="Hui-iconfont">&#xe665;</i> 查询数据</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="member_add('上传消费Excel','/admin/excelAdd?type=expense&warnning=确定添加所有的数据项到数据库吗？','500','250')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 上传消费Excel</a>
		</span> 
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				<td><input type="checkbox" value="<?php echo $exp->id;?>" name="id"></td>
				<td><?php echo isset($students[$exp->stu_id]->academyInfo->name)?$students[$exp->stu_id]->academyInfo->name:"";?></td>
				<td><?php echo isset($students[$exp->stu_id]->specialityInfo->name)?$students[$exp->stu_id]->specialityInfo->name:"";?></td>
				<td><?php echo isset($students[$exp->stu_id]->classInfo->name)?$students[$exp->stu_id]->classInfo->name:"";?></td>
				<td><?php echo $students[$exp->stu_id]->number;?></td>
				<td><?php echo $students[$exp->stu_id]->name;?></td>
				<td><?php echo $students[$exp->stu_id]->gender==0?'男':'女';?></td>
				<td><?php echo $exp->money;?></td>
				...
				<td class="td-manage">
					...
					<?php endif;*/?>
					<!-- <a title="编辑" href="javascript:;" onclick="member_edit('修改成绩数据','/admin/scoreEdit','<?php echo $exp->id;?>','500','510')" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6df;</i>
					</a>  -->
					...
					<a title="删除" href="javascript:;" onclick="member_del(this,'<?php echo $exp->id;?>')" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6e2;</i>
					</a>
				</td>
			</tr>
...
function member_del(obj,id){
	layer.confirm('确认要删除该条数据吗？',function(index){
	    ...
	    expense.infoType = 'expense';
	    expense.id = id;
		dataHandler('/common/deleteInfo',expense,null,null,null,function(){
...
function member_del_bulk(){
	...
	layer.confirm('确认要删除这些成绩数据吗？',function(index){
	    var expenses = new Object();
	    expenses.infoType = 'expenses';
	    expenses.idArray = memberArray;
	    dataHandler("/common/deleteBulkInfo",expenses,null,null,null,function(){
	    	$("input[name='id']:checked").each(function(){
